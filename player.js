...
class Player {
  constructor(divId, videoDetails) {
    ...
    this.channel = videoDetails.channel
...
class TwitchPlayer extends Player {
  ...
  seekTo(timestamp, targetState) {
    ...
    if (timestamp < this.startTime) {
      var durationSeconds = 1 // I think seek(0) does something wrong, so seeking to a unique offset
    ...
    // If we try to seek past the end time (and the end time is known), instead pause the video near the end.
      ...
      if (durationSeconds === 0) durationSeconds = 2 // I think seek(0) does something wrong, so seeking to a unique offset
    ...
    if (event == 'seek') {
      ...
      switch (this.state) {
        ...
        case ASYNC: // No action needed. The user is likely resuming the video so they can watch and sync it up.
          console.log('Ignoring unexpected play event for', this.id, 'was in state', this.state)
          break
      }
    } else if (event == 'pause') {
      switch (this.state) {
      ...
      switch (this.state) {
        ...
        case SEEKING_END:
        case AFTER_END:
          if (this.nextVideoDetails == null || this.nextVideoDetails.id == 0) {
